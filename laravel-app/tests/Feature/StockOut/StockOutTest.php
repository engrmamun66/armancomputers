...
namespace Tests\Feature\StockOut;
...
class StockOutTest extends TestCase
{
    ...
    public function test_can_create_stock_out_with_multiple_products_and_stock_decreases(): void
    {
        ...
        $response = $this->actingAs($user, 'api')->postJson('/api/stock-outs', [
    ...
    public function test_stock_out_rejects_insufficient_stock_and_does_not_change_stock(): void
    {
        $user = $this->makeUser(Role::ADMIN);
        $customer = $this->makeCustomer();
    ...
    public function test_stock_out_exactly_at_available_stock_succeeds(): void
    {
        $user = $this->makeUser(Role::ADMIN);
        $customer = $this->makeCustomer();
    ...
    public function test_fully_paid_stock_out_has_zero_due_and_paid_status(): void
    {
        $user = $this->makeUser(Role::ADMIN);
        $customer = $this->makeCustomer();
    ...
    public function test_deleting_stock_out_reverses_stock_and_cancels_invoice(): void
    {
        $user = $this->makeUser(Role::ADMIN);
        $customer = $this->makeCustomer();
        $product = $this->makeProduct(['current_stock' => 10]);

        $create = $this->actingAs($user, 'api')->postJson('/api/stock-outs', [
            'customer_id' => $customer->id,
            'sale_date' => now()->toDateString(),
            'payment_method' => 'cash',
            'items' => [['product_id' => $product->id, 'quantity' => 4, 'unit_price' => 50]],
        ]);
        $stockOutId = $create->json('data.id');
        $this->assertEquals(6, $product->fresh()->current_stock);

        $this->actingAs($user, 'api')->deleteJson("/api/stock-outs/{$stockOutId}")->assertOk();

        $this->assertEquals(10, $product->fresh()->current_stock);
        $this->assertSoftDeleted('stock_outs', ['id' => $stockOutId]);

        $invoice = Invoice::where('stock_out_id', $stockOutId)->first();
        $this->assertEquals('cancelled', $invoice->status->slug);
    }

    public function test_editing_stock_out_reconciles_stock_correctly(): void
    {
        $user = $this->makeUser(Role::ADMIN);
        $customer = $this->makeCustomer();
        ...
        $update = $this->actingAs($user, 'api')->putJson("/api/stock-outs/{$stockOutId}", [
            'customer_id' => $customer->id,
            'sale_date' => now()->toDateString(),
            'payment_method' => 'cash',
    ...
    public function test_editing_stock_out_rejects_when_new_quantity_exceeds_available_stock(): void
    {
        $user = $this->makeUser(Role::ADMIN);
        $customer = $this->makeCustomer();
        $product = $this->makeProduct(['current_stock' => 10]);
        ...
        $stockOutId = $create->json('data.id');
        // current_stock is now 5. Another sale (outside this Stock Out) consumes it entirely.
    ...
    public function test_staff_can_create_but_not_delete_stock_out(): void
    {
        $staff = $this->makeUser(Role::STAFF);
        $customer = $this->makeCustomer();
        $product = $this->makeProduct(['current_stock' => 10]);

        $create = $this->actingAs($staff, 'api')->postJson('/api/stock-outs', [
            'customer_id' => $customer->id,
            'sale_date' => now()->toDateString(),
            'payment_method' => 'cash',
            'items' => [['product_id' => $product->id, 'quantity' => 2, 'unit_price' => 50]],
        ...
        $stockOutId = $create->json('data.id');
        $this->actingAs($staff, 'api')->deleteJson("/api/stock-outs/{$stockOutId}")->assertStatus(403);
    }
}
