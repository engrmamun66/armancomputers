...
- `components/common/DateRangePicker.vue` wraps the third-party `em-datetimepicker` widget (`resources/js/vendor/em-datetimepicker/`, `components/common/EmDateTimePicker.vue`). Default mode is two separate date inputs; pass `unified` + `:presets="[...]"` for the single-field range-with-presets variant (used on Purchase/Sale/Invoice lists). `EmDateTimePicker` is the default for dates elsewhere (Purchase/Sale Date) — don't add a native `<input type="date">` for those. **Exception**: all Warranty End Date fields (`PurchaseForm.vue` header + per-item, `SaleForm.vue` per-item) deliberately use a plain `<input type="date">` instead — the `em-datetimepicker` widget kept self-defaulting to today's date on a fresh/empty field despite several attempted fixes, silently writing a wrong warranty date if the user didn't touch the field before saving. Values are plain `YYYY-MM-DD` strings either way, so no format conversion was needed to switch.
